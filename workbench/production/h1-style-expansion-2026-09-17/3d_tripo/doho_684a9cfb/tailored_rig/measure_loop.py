...
bpy.ops.import_scene.gltf(filepath=str(ROOT/'tailored_motion_loop.glb'))
...
(ROOT/'loop_before_contact.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
for t in ad.nla_tracks:t.mute=False
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'loop_before_review.blend'))
